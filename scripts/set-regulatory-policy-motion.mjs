 ...
 * to the Regulatory Development & Policy sector document, as real (editable) CMS data.
 *
 ...
 *
 * Run:   node --env-file=.env.local scripts/set-regulatory-policy-motion.mjs
 */
...
if (!projectId || !dataset || !token) {
  console.error(
    "Missing env. Need NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, SANITY_API_WRITE_TOKEN.\n" +
      "Run with: node --env-file=.env.local scripts/set-regulatory-policy-motion.mjs",
  );
  process.exit(1);
}
...
async function run() {
  ...
  console.log(
    `Adding In Motion + Careers + CTA to ${ids.length} regulatory-policy doc(s) in ${projectId}/${dataset}…`,
  );

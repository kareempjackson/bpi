/**
 * Clears every content section on the Research & Development sector document,
 * leaving only the header/hero.
 *
 * - Turns every section toggle off (showOverview/Practice/Capabilities/Stats/
 *   Quote/Motion).
 ...
 *
 * Run:   node --env-file=.env.local scripts/clear-research-development-sections.mjs
 */
...
if (!projectId || !dataset || !token) {
  console.error(
    "Missing env. Need NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, SANITY_API_WRITE_TOKEN.\n" +
      "Run with: node --env-file=.env.local scripts/clear-research-development-sections.mjs",
  );
  process.exit(1);
}
...
async function run() {
  ...
  console.log(
    `Clearing sections on ${ids.length} research-development doc(s) in ${projectId}/${dataset}…`,
  );

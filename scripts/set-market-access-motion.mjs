 ...
 * to the Market Access sector document, as real (editable) CMS data.
 *
 * - In Motion: light tone (navy text on the pale-blue canvas, convergence
 *   graphic on the right), with the three live initiatives from the approved
 *   screenshot.
 ...
 *
 * Run:   node --env-file=.env.local scripts/set-market-access-motion.mjs
 */
...
if (!projectId || !dataset || !token) {
  console.error(
    "Missing env. Need NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, SANITY_API_WRITE_TOKEN.\n" +
      "Run with: node --env-file=.env.local scripts/set-market-access-motion.mjs",
  );
  process.exit(1);
}
...
async function run() {
  ...
  console.log(
    `Adding In Motion + Careers + CTA to ${ids.length} market-access doc(s) in ${projectId}/${dataset}…`,
  );
  const tx = client.transaction();
  // unset any stale motionCta from the old seed so the header shows just "In Motion".

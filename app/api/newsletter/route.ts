...
export async function POST(request: Request) {
  ...
  // Deterministic doc id keyed off the email — re-subscribing with the
  // same address updates the existing record instead of creating a dupe.
  const emailHash = createHash("sha1").update(email).digest("hex").slice(0, 24);
  const docId = `newsletterSubscription.${emailHash}`;
